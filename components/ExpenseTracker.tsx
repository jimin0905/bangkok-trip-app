...
const ExpenseTracker = forwardRef<{ pushSettings: (settings: TripSettings) => Promise<void> }, Props>(({ isOpen, onRequestOpen, onRequestClose, tripSettings, onSettingsSync }, ref) => {
  ...
  const closeAllWidgets = () => { setShowSimpleCalc(false); setShowExchange(false); setShowUserManage(false); setShowAuth(false); };

  const toggleWidget = (type: 'calc' | 'exchange' | 'users' | 'auth') => {
      const isCurrentlyOpen = type === 'calc' ? showSimpleCalc : type === 'exchange' ? showExchange : type === 'users' ? showUserManage : showAuth;
      closeAllWidgets();
      if (!isCurrentlyOpen) {
  ...
  const isAnyWidgetOpen = showSimpleCalc || showExchange || showUserManage || showAuth;

  const toggleExpenseExpansion = (id: string) => {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 right-0 w-full sm:w-[450px] bg-[#F9F9F7] shadow-2xl z-50 transform transition-transform duration-300 flex flex-col border-l border-stone-200 overscroll-contain ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        {/* Header (固定) */}
        <div className={`px-6 pt-6 pb-6 text-white shadow-md relative flex-shrink-0 transition-colors duration-500 z-30 ${isSyncMode ? 'bg-emerald-900' : 'bg-stone-900'}`}>
          <div className="flex justify-between items-start relative z-10 mb-2">
              ...
              <div className="flex gap-2">
                  <button onClick={() => toggleWidget('calc')} className={`p-2 rounded-full transition-colors ${showSimpleCalc ? 'bg-orange-500 text-white shadow-lg' : 'bg-white/10 hover:bg-white/20'}`}><Calculator size={20} /></button>
                  <button onClick={() => toggleWidget('exchange')} className={`p-2 rounded-full transition-colors ${showExchange ? 'bg-orange-500 text-white shadow-lg' : 'bg-white/10 hover:bg-white/20'}`}><ArrowRightLeft size={20} /></button>
                  <button onClick={() => toggleWidget('users')} className={`p-2 rounded-full transition-colors ${showUserManage ? 'bg-white text-stone-900 shadow-lg' : 'bg-white/10 hover:bg-white/20'}`}><Users size={20} /></button>
                  {isSyncMode ? <button onClick={handleLogout} className="p-2 rounded-full bg-red-500/20 text-red-200 hover:bg-red-500/30"><LogOut size={20} /></button> : <button onClick={() => toggleWidget('auth')} className={`p-2 rounded-full transition-colors ${showAuth ? 'bg-emerald-500 text-white shadow-lg' : 'bg-white/10 hover:bg-white/20'}`}><CloudOff size={20} /></button>}
                      ...
                      <div key={u.id} className="flex-shrink-0 bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 min-w-[150px]">
                          ...
                          <div className="space-y-2">
                              <div className="flex flex-col">
                                  <span className="text-[10px] text-white/40 uppercase font-bold">代墊他人</span>
                                  <span className="text-sm font-bold font-mono">NT${(stats.userPaidForOthers[u.id] || 0).toLocaleString()}</span>
                              </div>
                              <div className="flex flex-col">
                                  <span className="text-[10px] text-white/40 uppercase font-bold">個人消費</span>
                     ...
                     <div className="bg-white text-stone-900 rounded-xl overflow-hidden shadow-lg animate-fade-in">
                         <button onClick={() => setShowDebtDetails(!showDebtDetails)} className="w-full flex items-center justify-between p-3 hover:bg-stone-50 transition-colors border-b border-stone-100">
                             <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-bold uppercase text-stone-500 tracking-wider">待結帳務</span>
                             </div>
                         ...
                         <div className={`transition-all duration-300 overflow-hidden ${showDebtDetails ? 'max-h-96 opacity-100 p-3' : 'max-h-0 opacity-0'}`}>
                             <div className="space-y-2.5 mb-3 text-stone-700">
        ...
        <div className="relative flex-1 min-h-0">
          <div id="expense-scroll-container" className="absolute inset-0 overflow-y-auto overscroll-contain no-scrollbar z-10 bg-[#F9F9F7]">
            {/* 記帳表單 */}
            <div className="bg-white border-b border-stone-100">
                <button onClick={() => (isFormExpanded && editingId ? resetForm() : setIsFormExpanded(!isFormExpanded))} className="w-full bg-white p-4 flex items-center justify-between hover:bg-stone-50 z-20 sticky top-0 shadow-sm border-b border-stone-50">
                    <div className="flex items-center gap-2">
                        <div className={`p-1.5 rounded-full ${isFormExpanded ? (editingId ? 'bg-orange-500 text-white' : 'bg-stone-800 text-white shadow-md') : 'bg-stone-100 text-stone-400'}`}>{editingId ? <Pencil size={14} /> : <Plus size={14} />}</div>
                ...
                <div className={`transition-all duration-300 ease-in-out relative z-10 ${isFormExpanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                    <div className="p-4 space-y-5 text-stone-800">
            ...
            <div className="p-4 space-y-4">
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2 sticky top-0 bg-[#F9F9F7]/95 backdrop-blur-sm z-20 py-2">
                    <button onClick={() => setFilterDate('all')} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${filterDate === 'all' ? 'bg-stone-800 text-white border-stone-800 shadow-md' : 'bg-white text-stone-400 border-stone-200 hover:border-stone-300'}`}>全部日期</button>
                ...
                <div className="space-y-3 pb-32">
                    {filteredExpenses.length === 0 ? (
                        ...
                        filteredExpenses.map(exp => {
                            ...
                            const payerName = users.find(u => u.id === exp.paidBy)?.name || '未指定';

                            return (
                                <div 
                                    key={exp.id} 
                                    onClick={() => toggleExpenseExpansion(exp.id)}
                                    className={`bg-white rounded-2xl overflow-hidden shadow-sm border border-stone-100 flex flex-col group relative transition-all cursor-pointer ${exp.isSettled ? 'opacity-60' : 'hover:border-stone-300 hover:shadow-md'}`}
                                >
                                    <div className="p-4 flex items-center gap-4">
                                        <div className={`w-12 h-12 flex-shrink-0 rounded-xl flex items-center justify-center ${cat.bg} ${cat.color} shadow-sm transition-transform duration-300 ${isExpanded ? 'scale-110' : ''}`}>
                                            <cat.icon size={20} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-0.5">
                                                <h4 className="font-bold text-stone-800 text-sm truncate">{exp.title}</h4>
                                                {exp.isSettled && <span className="flex-shrink-0 bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider">已結清</span>}
                                            </div>
                                            <div className="flex items-center gap-2 text-[10px] text-stone-400 font-bold uppercase tracking-wide">
                                                <span>{exp.date ? TRIP_DATES.find(d => d.value === exp.date)?.label : `DAY ${exp.dayId}`}</span>
                                                <span>•</span>
                                                <span className="flex items-center gap-1 text-stone-500 font-black"><User size={10}/> {payerName}</span>
                                            </div>
                                        </div>
                                        <div className="text-right flex-shrink-0 flex items-center gap-2">
                                            <div>
                                                <div className="text-sm font-black text-stone-800 font-mono">NT${exp.amount.toLocaleString()}</div>
                                                {exp.splitType === 'split' && <div className="text-[10px] text-stone-300 font-bold">與 {exp.involvedUsers.length} 人分</div>}
                                                {exp.splitType === 'self' && <div className="text-[10px] text-orange-400 font-bold">個人消費</div>}
                                                {exp.splitType === 'individual' && <div className="text-[10px] text-purple-400 font-bold">自訂分帳</div>}
                                            </div>
                                    ...
                                    <div className={`overflow-hidden transition-all duration-300 ease-in-out bg-stone-50/50 ${isExpanded ? 'max-h-96 border-t border-stone-100' : 'max-h-0'}`}>
                                        <div className="p-4 space-y-4">
                                            <div className="space-y-2">
                                                ...
                                                {exp.splitType === 'self' ? (
                                                    <div className="text-xs text-stone-500 bg-white p-2.5 rounded-lg border border-stone-100 flex items-center gap-2">
                                                        <Check size={14} className="text-orange-400" />
                                                        此筆為 <span className="font-bold text-stone-800">{payerName}</span> 的個人單獨支出。
                                                    </div>
                                                ) : (
                                                    <div className="grid grid-cols-1 gap-1.5">
                                                        {exp.involvedUsers.map(uid => {
                                                            const u = users.find(user => user.id === uid);
                                                            const isPayer = uid === exp.paidBy;
                                                            let shareAmount = 0;
                                                            
                                                            if (exp.splitType === 'split') {
                                                                shareAmount = Math.round(exp.amount / exp.involvedUsers.length);
                                                            } else if (exp.splitType === 'individual' && exp.individualAmounts) {
                                                                shareAmount = Number(exp.individualAmounts[uid] || 0);
                                                            }

                                                            return (
                                                                <div key={uid} className="flex items-center justify-between bg-white p-2.5 rounded-lg border border-stone-100 shadow-sm">
                                                                    <div className="flex items-center gap-2">
                                                                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] ${isPayer ? 'bg-stone-800 text-white' : 'bg-stone-100 text-stone-500'}`}>
                                                                            {isPayer ? <Receipt size={12} /> : <User size={12} />}
                                                                        </div>
                                                                        <span className={`text-xs ${isPayer ? 'font-bold text-stone-800' : 'text-stone-600'}`}>
                                                                            {u?.name} {isPayer && <span className="text-[10px] text-stone-400 font-normal">(付款者)</span>}
                                                                        </span>
                                                                    </div>
                                                                    <div className="text-right">
                                                                        <span className="text-xs font-bold font-mono text-stone-700">NT${shareAmount.toLocaleString()}</span>
                                                                    </div>
                                                                </div>
                                                            );
                                                        })}
                                            ...
                                            {!exp.isSettled && (
                                                <div className="flex gap-2 pt-2 border-t border-stone-200/50">
                                                    <button onClick={(e) => handleEdit(exp, e)} className="flex-1 bg-white border border-stone-200 py-2 rounded-lg text-xs font-bold text-stone-600 hover:bg-stone-50 flex items-center justify-center gap-2 active:scale-95 transition-all"><Pencil size={12}/> 編輯金額</button>
                                                    <button onClick={(e) => handleDelete(exp.id, e)} className="flex-1 bg-white border border-red-100 py-2 rounded-lg text-xs font-bold text-red-500 hover:bg-red-50 flex items-center justify-center gap-2 active:scale-95 transition-all"><Trash2 size={12}/> 刪除紀錄</button>
                                                </div>
                                            )}
                                        </div>
                                    </div>
          ...
          {/* 懸浮工具疊層 */}
          {isAnyWidgetOpen && <div className="absolute inset-0 bg-stone-900/10 backdrop-blur-[2px] z-[55] animate-fade-in" onClick={closeAllWidgets} />}
          
          <div className="absolute top-4 left-4 right-4 z-[60] pointer-events-none">
              {showSimpleCalc && (
                  <div className="bg-stone-800 rounded-2xl p-4 animate-fade-in shadow-2xl border border-stone-700 ring-4 ring-black/5 pointer-events-auto">
                      ...
                      <div className="bg-stone-900 rounded-lg p-3 mb-3 text-right"><span className="text-2xl font-mono font-bold break-all text-white">{simpleCalcDisplay || '0'}</span></div>
                      <div className="grid grid-cols-4 gap-2">{calcButtons.map(btn => (<button key={btn.val} onClick={() => handleSimpleCalcInput(btn.val)} className={`p-3 rounded-lg font-bold text-lg transition-colors flex items-center justify-center ${btn.wide ? 'col-span-2' : ''} ${(btn as any).tall ? 'row-span-2' : ''} ${btn.bg || 'bg-stone-700/50 hover:bg-stone-700 text-white'}`}>{btn.icon ? <btn.icon size={20} /> : btn.label}</button>))}</div>
                  </div>
              )}

              {showExchange && (
                  <div className="bg-white rounded-2xl p-4 animate-fade-in shadow-2xl border border-stone-200 ring-4 ring-black/5 pointer-events-auto text-stone-800">
                      ...
                      <div className="flex gap-2 mb-3">
                          {['THB', 'MYR'].map(c => (<button key={c} onClick={() => setCalcCurrency(c as any)} className={`flex-1 py-1.5 text-xs font-bold rounded-lg border transition-all ${calcCurrency === c ? 'bg-orange-100 border-orange-200 text-orange-700 shadow-sm' : 'bg-stone-50 border-stone-100 text-stone-400'}`}>{c === 'THB' ? '泰銖' : '馬幣'}</button>))}
                      </div>
              ...
              {showUserManage && (
                  <div className="bg-stone-800 rounded-2xl p-4 animate-fade-in shadow-2xl border border-stone-700 ring-4 ring-black/5 pointer-events-auto text-white">
                      <div className="flex justify-between items-center mb-3 text-stone-400">
                      ...
                      <div className="space-y-2 mb-4 max-h-[150px] overflow-y-auto no-scrollbar">
                          {users.map(u => (<div key={u.id} className="flex items-center justify-between bg-white/5 px-3 py-2 rounded-lg border border-white/5"><span className="text-sm font-bold">{u.name}</span><button onClick={() => handleRemoveUser(u.id)} className="text-white/20 hover:text-red-400 transition-colors"><MinusCircle size={16} /></button></div>))}
                      </div>
                      <div className="flex gap-2">
                          <input type="text" value={newUserName} onChange={e => setNewUserName(e.target.value)} placeholder="新成員名稱" className="flex-1 bg-white/10 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none" />
